import { useEffect, useState } from "react";
import { createFileRoute, Link } from "@tanstack/react-router";
import { BookOpen, Building2, ContactRound, MessageCircle, Send } from "lucide-react";

import { AdminError, AdminShell } from "@/components/admin/AdminShell";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { useNeonAuth } from "@/hooks/use-neon-auth";
import { fetchAdminOverview } from "@/lib/neon/admin-data";

type Overview = Awaited<ReturnType<typeof fetchAdminOverview>>;

export const Route = createFileRoute("/admin/")({
  head: () => ({
    meta: [{ title: "Admin｜Earnest Property" }, { name: "robots", content: "noindex" }],
  }),
  component: AdminHome,
});

function AdminHome() {
  const { user } = useNeonAuth();
  const [data, setData] = useState<Overview | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!user) return;
    fetchAdminOverview()
      .then(setData)
      .catch((err) => setError(err instanceof Error ? err.message : String(err)));
  }, [user]);

  return (
    <AdminShell title="總覽" description="CMS、CRM、WhatsApp 及群發工作台。">
      {error ? <AdminError message={error} /> : null}
      {!data && !error ? (
        <Skeleton className="h-64 w-full" />
      ) : data ? (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <MetricCard icon={Building2} label="放盤" value={data.properties} to="/admin/listings" />
          <MetricCard
            icon={ContactRound}
            label="跟進中 leads"
            value={data.openLeads}
            to="/admin/leads"
          />
          <MetricCard icon={BookOpen} label="聯絡人" value={data.contacts} to="/admin/leads" />
          <MetricCard
            icon={MessageCircle}
            label="WhatsApp 對話"
            value={data.openConversations}
            to="/admin/whatsapp"
          />
          <MetricCard
            icon={Send}
            label="進行中群發"
            value={data.activeCampaigns}
            to="/admin/blasts"
          />
        </div>
      ) : null}

      <section className="mt-5 grid gap-4 lg:grid-cols-3">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">今日優先</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>1. 處理新查詢及 WhatsApp 對話。</p>
            <p>2. 更新放盤狀態、相片及 SEO 欄位。</p>
            <p>3. 審核任何準備群發的 WhatsApp template campaign。</p>
          </CardContent>
        </Card>
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">Woztell 狀態</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Live sending 由 `WOZTELL_ENABLED` 控制；未配置 token / channel secret
            前，後台仍可管理對話與 campaign draft。
          </CardContent>
        </Card>
      </section>
    </AdminShell>
  );
}

function MetricCard({
  icon: Icon,
  label,
  value,
  to,
}: {
  icon: React.ComponentType<{ className?: string }>;
  label: string;
  value: number;
  to: "/admin/listings" | "/admin/leads" | "/admin/whatsapp" | "/admin/blasts";
}) {
  return (
    <Link to={to}>
      <Card className="h-full transition hover:border-primary/50 hover:shadow-sm">
        <CardContent className="flex items-center justify-between gap-3 p-4">
          <div>
            <p className="text-sm text-muted-foreground">{label}</p>
            <p className="mt-1 text-2xl font-semibold">{value.toLocaleString()}</p>
          </div>
          <Icon className="h-5 w-5 text-primary" />
        </CardContent>
      </Card>
    </Link>
  );
}
