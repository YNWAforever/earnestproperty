...
import { retryJob } from "../lib/control-plane/jobs.server.ts";
...
function conflictError() {
  return Object.assign(new Error("The job cannot be retried from its current state."), {
    code: "CONFLICT_DUPLICATE",
  });
}

export const Route = createFileRoute("/api/admin/control-plane/jobs/$id/retry")({
  server: {
    handlers: {
