    ...
    <Container className="py-10">
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
        ...
        <div className="min-w-0 rounded-lg border bg-card p-5">
          ...
          {psfTrend.length >= 2 && (
            <div className="mt-4">
              <p className="text-xs text-muted-foreground">實呎走勢（{psfTrend.length} 宗成交）</p>
              <div className="mt-2 h-48 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={psfTrend} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                    <XAxis
                      dataKey="date"
                      stroke="var(--muted-foreground)"
                      fontSize={11}
                      tickFormatter={(value: string) => value.slice(2, 10).replace(/-/g, "/")}
                    />
                    <YAxis
                      stroke="var(--muted-foreground)"
                      fontSize={11}
