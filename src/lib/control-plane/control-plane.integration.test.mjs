...
test(
  "control plane migration applies and protects audit rows in a disposable database",
  {
  ...
  async () => {
    ...
    try {
      ...
      await assert.rejects(
        () =>
          queryRows("UPDATE ops_audit_logs SET action = $1 WHERE request_id = $2::uuid", [
            "changed",
            requestId,
          ]),
        /append-only/i,
      ...
      const transactionResult = await transactionRows([
...
test(
  "migration service rejects stale approvals and records one successful apply",
  {
    skip: !process.env.TEST_DATABASE_URL,
  },
  async () => {
...
test(
  "durable jobs are idempotent, claimed disjointly, and recover expired leases",
  {
    skip: !process.env.TEST_DATABASE_URL,
  },
  async () => {
    ...
    try {
      ...
      const { cancelJob, claimJobs, completeJob, enqueueJob, recoverExpiredLeases } =
      ...
      await queryRows(
        "DELETE FROM ops_jobs WHERE idempotency_key LIKE 'control-plane-integration:%'",
      );
      ...
      assert.equal(
        workerB.some((job) => workerAIds.has(job.id)),
        false,
      );
      assert.deepEqual(
      ...
      assert.equal(
        recovered.some((job) => job.id === expired.id),
        true,
      );
      assert.equal(
        recovered.some((job) => job.id === live.id),
        false,
      );
