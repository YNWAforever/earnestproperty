...
test("repository safety helpers reject unsafe metadata and map only the named conflict", () => {
  ...
  assert.deepEqual(
    sanitizeContentCopilotAuditMetadata({ provider: "opencode_go", model: "go-content", latencyMs: 12, acceptedFields: ["title_zh"] }, "listing"),
  ...
  assert.throws(() => sanitizeContentCopilotAuditMetadata({ acceptedFields: ["price"] }, "listing"), /COPILOT_AUDIT_METADATA_INVALID/);
