...
async function fetchResource(queryRows: QueryRows, request: ContentCopilotRequest, actor: StaffAccess) {
  const privileged = actor.roles.includes("admin") || actor.roles.includes("manager");
  if (request.resourceType === "estate") {
    return first(queryRows(
...
function mapResource(type: ContentCopilotResourceType, row: Row) {
  ...
  if (type === "estate") {
    Object.assign(resource, pick(row, ["slug", "district_slug", "developer", "year_completed", "phases", "total_units", "area_min", "area_max", "facilities", "updated_at"]));
  } else if (type === "article") {
    Object.assign(resource, pick(row, ["slug", "category", "reading_minutes", "published", "published_at", "updated_at"]));
  } else if (type === "faq") {
    Object.assign(resource, pick(row, ["scope", "sort_order", "created_at"]));
  } else if (type === "video") {
    Object.assign(resource, pick(row, ["video_url", "sort_order", "published", "created_at", "updated_at"]));
  } else {
    Object.assign(resource, pick(row, ["listing_no", "title_en", "deal_type", "price", "rent", "saleable_area", "gross_area", "bedrooms", "bathrooms", "status", "district_slug", "estate_id", "estate_slug", "estate_name_zh", "estate_district_slug", "agent_name_zh", "agent_name_en", "updated_at"]));
  }
  return resource;
}

function buildSearchQuery(type: ContentCopilotResourceType, resource: Record<string, unknown>) {
  const values = [resource.title, resource.title_zh, resource.name_zh, resource.question, resource.estate_name_zh, resource.district_slug]
...
function mapKnowledgeEvidence(chunks: unknown[]) {
  ...
  for (const chunk of chunks.slice(0, 6)) {
    ...
    const rawUrl = typeof row.url === "string" ? row.url : typeof row.url_path === "string" ? row.url_path : null;
    evidence.push({
