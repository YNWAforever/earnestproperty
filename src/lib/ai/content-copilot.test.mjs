...
test("fingerprints are stable across object key order and change with source content", async () => {
  ...
  const reordered = await buildContentFingerprint({
    content: "first content",
    title: "first title",
  });
  const changed = await buildContentFingerprint({
    title: "first title",
  ...
  const result = applySelectedContentPatches(
    ...
    [
      ...
      },
      {
        ...
        confidence: "high",
        evidenceIds: [],
        unsupportedClaims: ["uncited travel time"],
      },
    ],
...
test("patch application rejects stale, conflicting, and non-allowlisted changes", () => {
  ...
  assert.equal(
    applySelectedContentPatches({}, [patch], ["title_zh"], {
      resourceType: "listing",
      sourceFingerprint: "a".repeat(64),
  ...
  assert.equal(
    applySelectedContentPatches({ title_zh: "changed" }, [patch], ["title_zh"], {
      resourceType: "listing",
      sourceFingerprint: "a".repeat(64),
      currentFingerprint: "a".repeat(64),
    }).error,
  ...
  assert.equal(
    applySelectedContentPatches({ price: "1" }, [{ ...patch, field: "price" }], ["price"], {
      resourceType: "listing",
      sourceFingerprint: "a".repeat(64),
      currentFingerprint: "a".repeat(64),
    }).error,
...
test("web evidence requires an https citation and factual web claims require a citation", () => {
  const missingCitation = validateContentCopilotProposal({
  ...
  const unsafeCitation = validateContentCopilotProposal({
    resourceType: "article",
    sourceFingerprint: "a".repeat(64),
    patches: [],
    evidence: [
      { id: "web-1", type: "web", title: "Unsafe", url: "http://example.com", excerpt: "x" },
    ],
    warnings: [],
  });
  ...
  assert.equal(
    contentCopilotRequestSchema.safeParse({
      resourceType: "article",
