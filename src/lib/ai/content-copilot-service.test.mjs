...
const articleRequest = { resourceType: "article", resourceId: "11111111-1111-4111-8111-111111111111", action: "improve", selectedFields: ["title"], tone: "professional_property", targetLanguage: "zh-HK", researchMode: "internal" };
const listingRequest = { resourceType: "listing", resourceId: "22222222-2222-4222-8222-222222222222", action: "improve", selectedFields: ["title_zh"], tone: "professional_property", targetLanguage: "zh-HK", researchMode: "internal" };

async function makeArticleContext() {
  const resource = { id: articleRequest.resourceId, title: "Sham Tseng market guide", description: "", district_slug: "sham-tseng" };
  return { resource, internalEvidence: [], query: resource.title };
}

async function makeProposal(resource) {
  return { resourceType: "article", sourceFingerprint: await buildContentFingerprint(resource), patches: [], evidence: [], warnings: [] };
}
...
function makeServiceDeps(overrides = {}) {
  return {
    ...
    generate: async ({ context }) => ({ ok: true, value: await makeProposal(context.resource), model: "go-content", latencyMs: 10, usageMetadata: {}, error: null }),
...
test("generation reloads authoritative record and excludes CRM data from prompts", async () => {
  ...
  const service = createContentCopilotService({
    ...
    generate: async ({ prompt, context }) => {
      ...
      return { ok: true, value: await makeProposal(context.resource), model: "go-content", latencyMs: 10, usageMetadata: {}, error: null };
    },
  });
...
test("listing agents cannot generate against another agent listing", async () => {
  const service = createContentCopilotService({ ...makeServiceDeps(), loadContext: async () => { throw new Response("Forbidden", { status: 403 }); } });
  await assert.rejects(service.generateContentProposal(listingRequest, agentActor), (error) => error instanceof Response && error.status === 403);
});
...
test("web mode attaches Tavily citations to generation", async () => {
  ...
  const result = await service.generateContentProposal({ ...articleRequest, researchMode: "web" }, managerActor);
  assert.equal(result.ok, true);
...
test("provider failure marks proposal failed and clears the generating lease", async () => {
  let failedId = "";
  const service = createContentCopilotService({ ...makeServiceDeps(), failProposal: async ({ proposalId }) => { failedId = proposalId; }, generate: async () => ({ ok: false, value: null, model: "go-content", latencyMs: 20, usageMetadata: {}, error: "OPENCODE_GO_GENERATION_FAILED" }) });
  const result = await service.generateContentProposal(articleRequest, managerActor);
...
test("decision rejects stale fingerprints before recording applied status", async () => {
  ...
  const service = createContentCopilotService({
    ...makeServiceDeps(),
    getProposal: async () => ({ id: "proposal-1", resourceType: "article", resourceId: articleRequest.resourceId, action: "improve", sourceFingerprint: "00".repeat(32), status: "generated", expiresAt: new Date(Date.now() + 60000).toISOString(), selectedFields: ["title"], patches: [] }),
