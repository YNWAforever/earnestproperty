...
test("proposal migration enforces lifecycle, quota indexes, and staff ownership", () => {
  ...
  assert.match(
    sql,
    /status IN \('generating','generated','partially_applied','applied','rejected','expired','failed'\)/,
  );
...
test("repository exposes explicit proposal transitions and AI audit writes", () => {
  ...
  for (const name of [
    "startContentProposal",
    "completeContentProposal",
    "failContentProposal",
    "getContentProposal",
    "decideContentProposal",
    "writeContentCopilotAudit",
  ]) {
